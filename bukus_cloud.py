...
def parse_books_from_page(page=1):
    ...
    for h1 in soup.find_all("h1"):
        link = h1.find("a", href=re.compile(r"/knjiga/\d+"))
        if not link:
            continue
        ...
        # Author <p> has class "text-lg", unlike other <p> tags which use "text-sm"
        author = ""
        parent = h1.parent
        if parent:
            p_tag = parent.find("p", class_=lambda c: c and "text-lg" in c)
            if p_tag:
