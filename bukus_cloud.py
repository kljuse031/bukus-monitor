...
BUKUS_USERNAME = os.environ.get("BUKUS_USERNAME")
BUKUS_PASSWORD = os.environ.get("BUKUS_PASSWORD")
...
def get_session():
    ...
    r = s.get("https://bigusbukus.com/korisnici/sign_in", timeout=10)
    ...
    if not csrf:
        print(f"Could not find CSRF token. Page length: {len(r.text)}")
        print(f"Page snippet: {r.text[:500]}")
        return

    resp = s.post(
        "https://bigusbukus.com/korisnici/sign_in",
        data={
            "user[username]": BUKUS_USERNAME,
            "user[password]": BUKUS_PASSWORD,
            "authenticity_token": csrf
        },
        timeout=10
    )
    logged_in = "sign_out" in resp.text or "odjava" in resp.text.lower()
    print(f"Login {'successful' if logged_in else 'FAILED'}")
    session = s
